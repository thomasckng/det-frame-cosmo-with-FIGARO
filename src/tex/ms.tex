...
The uncertainty is larger than the one reported in \cite{LIGOScientific:2021aug} ($H_0 = 50^{+37}_{-30} \, \mathrm{km/s/Mpc}$), .
...
This means a significant fraction of the reconstructed observed population distribution has an optimized $H_0$ around $140\, \mathrm{km/s/Mpc}$, which does not include the uncertainty from the mismatch between the population model and the observed population distribution.
The reason for the peak differing from the one reported in \cite{LIGOScientific:2021aug}, which is around $50\, \mathrm{km/s/Mpc}$, .
