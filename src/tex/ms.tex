...
This procedure gives us a set of optimized $\Lambda_i$ and $\Omega_i$ for each $\Theta_i$.
...
To prepare the mock data, we first generated $10000$ sets of $m_1$ and $z$ samples from a \textsc{power law + peak} distribution (see, e.g., Eq.~B4 in \cite{KAGRA:2021duu}) for the $m_1$ distribution and uniform distribution over comoving volume and source-frame time (see, e.g., Eq.~11 in \cite{LIGOScientific:2019zcs}) for the $z$ distribution.
...
In this subsection, we first test our method with a simplified case where we only infer $H_0$.
