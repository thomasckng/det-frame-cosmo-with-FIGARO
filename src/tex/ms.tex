...
We included all $70$ events labeled as BBH in Table~I of \cite{KAGRA:2021duu}, these correspond to all the events with $\mathrm{FAR_{min}} < 1\ \mathrm{yr}^{-1}$, where $\mathrm{FAR_{min}}$ is the smallest false alarm rate among all search pipelines.
...
\end{figure}
The result implies that \variable{output/real_H0.txt}, which has an uncertainty much smaller than $H_0 = 50^{+37}_{-30}$ reported in \cite{KAGRA:2021duu}.
This is mainly because the uncertainty only includes the uncertainty from the reconstruction, which underestimates the true uncertainty which also includes the mismatch between the population model and the observed population distribution.
