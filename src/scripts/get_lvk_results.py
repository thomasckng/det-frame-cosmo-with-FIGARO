...
event_median = np.array(med_all)
np.savetxt(paths.data/'real/samples_med.txt', event_median)
np.savetxt(paths.data/'real/jsd_bounds.txt', [[np.min(event_median[:, 0]), np.max(event_median[:, 0])], [np.min(event_median[:, 1]), np.max(event_median[:, 1])]])
